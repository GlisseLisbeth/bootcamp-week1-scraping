  ...
  for(let x = 0; x < sc.length; x++){
    ...
    if(header.toLowerCase().replace(/[^a-z]/gi,'').match('experiencia')) {
      ...
      for (let i = 0; i < li_array.length; i++) {
          ...
          let stitle = course(d2.children[0]);
          let from = '';
          let till = '';
          if(d2.children[1]){
...
const scrapingProfile = () => {
  ...
  wait(2000).then(function () {
    if(document.getElementsByClassName('profile-detail').length > 0){
      ...
      let porfile = sectionInfo.getElementsByClassName('break-words');
      userData.personal['name'] = porfile[0].innerText;
      userData.personal['title'] = porfile[1].innerText;
      wait(2000);
      const elementResumme = document.getElementById(
        "line-clamp-show-more-button"
      );
      if (elementResumme) elementResumme.click();
